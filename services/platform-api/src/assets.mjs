...
export const PUBLIC_ASSET_KINDS = new Set(["modeled", "editorial", "flatlay", "thumbnail"]);
...
async function requireOwner(pool, projectId, garmentId, outfitId) {
  ...
  if (garmentId) {
    const result = await pool.query("SELECT 1 FROM garments WHERE project_id=$1 AND id=$2", [projectId, garmentId]);
    if (!result.rowCount) throw new ApiError(422, "ASSET_OWNER_INVALID", "Garment owner not found");
  }
  if (outfitId) {
    const result = await pool.query("SELECT 1 FROM outfits WHERE project_id=$1 AND id=$2", [projectId, outfitId]);
    if (!result.rowCount) throw new ApiError(422, "ASSET_OWNER_INVALID", "Outfit owner not found");
  }
}
...
function intentResponse(storage, row) {
  ...
  return {
    ...
    upload: {
      method: "PUT",
      path: `/uploads/${row.id}?expires=${signed.expires}&signature=${signed.signature}`,
...
export async function promoteAsset(pool, storage, projectId, assetId, input, actor = "admin") {
  ...
  return withTransaction(pool, async client => {
    ...
    if (!PUBLIC_ASSET_KINDS.has(row.kind)) throw new ApiError(422, "ASSET_KIND_NOT_PUBLIC", `Asset kind ${row.kind} cannot be published`);
