...
CREATE TABLE migration_runs(
 ...
 UNIQUE(project_id,source_fingerprint,dry_run),
 CHECK(jsonb_typeof(inventory)='object'),
 CHECK(jsonb_typeof(before_counts)='object'),
 CHECK(jsonb_typeof(after_counts)='object'),
...
CREATE TABLE migration_entities(
 ...
 source_checksum text CHECK(source_checksum IS NULL OR source_checksum ~ '^[0-9A-Fa-f]{64}$'),
 target_checksum text CHECK(target_checksum IS NULL OR target_checksum ~ '^[0-9A-Fa-f]{64}$'),
